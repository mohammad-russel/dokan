...
<?php } elseif ($data == $month) { ?>
    <?php
    $sql = "SELECT `ymd`, SUM(price * quantity) as total_bill FROM cart WHERE `status` = 'complete' AND sr = $sid AND `ymd` >= DATE_SUB(CURRENT_DATE(), INTERVAL 30 DAY) GROUP BY ymd ORDER BY `ymd` DESC";
    $result = mysqli_query($con, $sql);
...
<?php } elseif ($data == $year) { ?>
    <?php
    $sql = "SELECT `ymd`, SUM(price * quantity) as total_bill FROM cart WHERE `status` = 'complete' AND sr = $sid AND `ymd` >= DATE_SUB(CURRENT_DATE(), INTERVAL 365 DAY) GROUP BY ymd ORDER BY `ymd` DESC";
    $result = mysqli_query($con, $sql);
